...
module.exports = async(client, message) => {
            ...
            if (cmd.length === 0) {
                if (matchedPrefix.includes(client.user.id))
                    return message.channel.send(new Discord.MessageEmbed()
                        ...
                        .setTitle(`Bão? Me chamou?`)
                        .setDescription(`Pra ver tudo só digitar \`${prefix}help\` uai`)
                    );
                return;
            }
                ...
                if (timestamps.has(message.author.id)) { //if the user is on cooldown
                    ...
                    if (now < expirationTime) { //if he is still on cooldonw
                        ...
                        return message.channel.send(new Discord.MessageEmbed()
                            ...
                            .setFooter(ee.footertext, ee.footericon)
                            .setTitle(`❌ Espera ${timeLeft.toFixed(1)} segundos antes de usar o comando \`${command.name}\` de novo sô!.`)
                ...
                try {
                    ...
                    if (command.memberpermissions && !message.member.hasPermission(command.memberpermissions, { checkAdmin: command.adminPermOverride, checkOwner: command.adminPermOverride })) {
                        return message.channel.send(new Discord.MessageEmbed()
                                .setColor(ee.wrongcolor)
                                .setFooter(ee.footertext, ee.footericon)
                                .setTitle("❌ Erro | Cê num pode fazer isso não!")
                                .setDescription(`Você precisa desses negócio aqui sô: \`${command.memberpermissions.join("`, ``")}\``)
        ...
        if(!message.guild.me.hasPermission(required_perms)){
          ...
          return message.channel.send(new Discord.MessageEmbed()
            .setColor(ee.wrongcolor)
            .setFooter(ee.footertext, ee.footericon)
            .setTitle("❌ Erro | Eu num tenho permissão não uai!")
            .setDescription("Me dá ADM aí zz.\n Ou então só dar esses trem que eu preciso: \n> `" + required_perms.join("`, `") +"`")
          )
        }
        ...
        return message.channel.send(new Discord.MessageEmbed()
          .setColor(ee.wrongcolor)
          .setFooter(ee.footertext, ee.footericon)
          .setTitle("❌ Deu algum trem errado enquanto eu tava rodando o comando `" + command.name + "`")
    ...
    return message.channel.send(new Discord.MessageEmbed()
      .setColor(ee.wrongcolor)
      .setFooter(ee.footertext, ee.footericon)
      .setTitle(`❌ Sei que isso não, escreve **\`${prefix}help\` pra ver os trem**`)
      .setDescription(`Se quiser saber ainda mais, escreva \`${prefix}help [nome do comando]\``)
    ).then(msg=>msg.delete({timeout: 5000}).catch(e=>console.log("Couldn't Delete --> Ignore".gray)));
  }catch (e){
    return message.channel.send(
    new MessageEmbed()
    .setColor("RED")
    .setTitle(`❌ Erro | Deu um trem aqui`)
